...
**Subject-name bonus** (Stage 2 output): `subjectName` + `nameStrength`. Если `nameStrength >= 60` → бонус до +10 к memePotential. Threshold +15 storyBonus + +10 nameBonus = **+25 cap бустеров**, штрафы отдельно (multiplicative penaltyMult по buzz/momentum/organicity).
...
- **whyNow** — конкретный триггер события (cap 280 chars, может быть пустой)
