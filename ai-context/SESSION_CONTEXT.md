...
**Single source of truth для прав плана** — `src/billing/entitlements.js`. `getPlanEntitlements(planName)` → `{sources, manualAnalyze, catalyst, xAnalysis, historyHours, favorites}`. Caps semantics: `-1` = unlimited (admin везде; pro тоже `-1` по `xAnalysis`/`historyHours`), `0` = blocked (free), `N>0` = N/day soft-cap (in-memory rolling 24h). Импортируется и в dashboard/server.js, и в telegram.js — одинаковые правила на сайте и в боте.

- `historyHours: 72` для free (3-day window cap), `-1` для остальных.
- `favorites: true` для pro/admin.
- `xAnalysis` (X/Twitter on-demand analyze — 🔍-кнопка под алертом, live-метрики твита через Apify): free=0, test=10/день, pro/admin=`-1`.
...
- **Save → Alerts** — запись в БД (NB: `save_only`-тренды пишутся ДО Stage 1, scored — после Stage 2; см. §Alert gate «AI-score gate») + alert-loop с per-user gate.
...
**Endpoints** (admin only): `GET /api/preset-configs` → `{ defaults, effective, overrides, autoOverrides, tagsLocked, fieldRanges, presets, groups }` · `POST /api/preset-configs { overrides, tagsLocked? }`. Save с empty `overrides` стирает только manual layer — auto-blob НЕ трогается (старая panic Clear-ALL semantic убрана 2026-05-12, заменена явными кнопками 🧹 Wipe manual / ↩ Restore hardcoded, см. выше).
...
- **rankScore** = `e*0.15 + a*0.85` ± feedback bias (adoption доминирует; default sort в дашборде).
...
- kpop/fandom: +30
- celeb-noise: +20
- no-meme-shape: +20 (general), 10..25 per-preset.
- no-content (text-only): +12 (general), 0..15 per-preset. Skipped когда **все** items кластера от source'а без медиа by design (`google_trends`).

**Safe-signal override**: animal/absurd/meme/heartwarming → делим raw на 3 (или 4 при ≥2 сигналах). Срабатывает только если `raw > 0`. **NB**: text-only penalty НЕ входит в raw → НЕ делится на divisor (прибавляется уже после override).

**Meme-shape boost** (per-preset additive bonus к `emergenceScore`): general 10, animals 14, culture 12, celebrities 10, events 4. `perSignalBoost * (signalCount >= 2 ? 1.5 : 1)`. Применяется в clusterer до routing.
...
- xAI curated: `grok-4-1-fast-reasoning`, `grok-4-1-fast-non-reasoning`, `grok-4-fast-non-reasoning`, `grok-4.20-0309-reasoning`, `grok-4.20-0309-non-reasoning`, `grok-3-mini`.
- OpenAI curated: `gpt-4.1-mini/.1`, `gpt-4o-mini/4o`, `gpt-5-mini/5`. (Дефолт в коде `gpt-5.4-mini` — вне curated, задаётся через DB/env.)
- Gemini curated (3-й HTTP-провайдер, list захардкожен — Google `/v1beta/openai/models` отдаёт сотни internal IDs): `gemini-3.1-flash-lite`, `gemini-3.1-flash-lite-preview`, `gemini-2.5-flash`, `gemini-2.5-flash-lite`.
...
- **Nano** (`gpt-5.4-nano` через OPENAI_API_KEY) — text-only enrichment. **По умолчанию ВЫКЛЮЧЕН** (`nanoEnabled='0'` с 2026-05-09 — стал dead weight после Stage 1 rework на Grok/Gemini, его поля дублирует новый Stage 1; re-enable из админки для A/B). Output: `topicSummary`, `entityCanonical[]`, `language`, `slangDecoded`. Inputs: title + description (cap 600) + r/sub + #hashtag + by @author + RelatedPosts (sibling titles из cluster, top 5 by engagement).
...
- `stage2MaxCalls=6` (DB-tunable, UI/scanner default; code-fallback в `scorer.js` = 3) — cap вызовов на цикл.
...
Per-actor tokens: `APIFY_API_KAITO`, `APIFY_API_XQUIK`. Generic: `APIFY_API`. `buildInput(query, maxItems)` actor-agnostic — оба актора получают `searchTerms: [<string>]` (Kaito-поле `twitterContent` ломало валидацию на части ревизий схемы → ушли на `searchTerms`). Output идентичный.
...
**Видео-файл**:
- **apidojo** отдаёт работающий `videoUrl` (URL с подписанным токеном, валиден ~6h). `_normalize` через `_firstNonAudioUrl` пробрасывает в `metrics.videoUrl` с приоритетом `video.video?.url` (nested, primary) `> videoUrlNoWaterMark > videoUrl > mediaUrls[0]`. Stage 0 Gemini-captioner качает нативно.
...
**Live hashtag discovery** (Stage A для TikTok): раз в 12h (`TIKTOK_TRENDS_REFRESH_MINUTES=720`) collector дёргает `clockworks~tiktok-trends-scraper` actor (по slug; numeric ID в коде не используется) — зеркало TikTok Creative Center (официальная публикация trending hashtags). Top-24 (`TIKTOK_TRENDS_TOP_N=24`) с `isPromoted=false`, `videoCount > 0`. Кэш в `settings.tiktokTrendingHashtags` (JSON `{hashtags:[], fetchedAt}`). Live-discovery — primary path в `_getHashtags()`; per-preset hardcoded list — fallback. Стоимость ~$4.32/мес.
...
- Google Trends → search traffic (отдельная строка из `m.formattedTraffic` через `alertGoogleTraffic`)

i18n keys: `alertLikes`, `alertPlays`, `alertUpvotes`, `alertGoogleTraffic`, `alertComments` (EN + RU). (`alertGoogleHits` в словаре есть, но в `formatter.js` НЕ вызывается — Google идёт через `alertGoogleTraffic`.)

## Source icons

Иконки источников через центральный `icon()` helper (Lucide / custom `ICONS` registry) — R4-редизайн (2026-05-20) выпилил прежние simpleicons-SVG-через-`dangerouslySetInnerHTML`.

`SOURCE_ICONS` маппит source-id → icon-name: `{reddit:'reddit', google_trends:'google', twitter:'twitter', tiktok:'tiktok', x_trends:'hash'}` (значения — ICON-KEY строки, не глифы; раньше были letter-mark R/G/𝕏/♪/#). `SourceMark({ src, fallback, size })` резолвит имя через `icon(SOURCE_ICONS[src])`; unknown source → центрированная точка `·`. CSS `.src-mark-svg { width/height: 60% }` от родительского чипа, Twitter 56% (тонкий по природе).
...
2. **Dashboard** — bottom-nav «🧪 Анализ» (только pro/admin) → Sheet с AnalyzePanel. `save:true` — попадает в shared feed с бейджем «🧪 MANUAL» (Bundle #2, 2026-06-07; раньше было `save:false`/приватно). TG-рассылки нет — её делает только admin-path. Rate limit: 30s между + дневной cap из entitlements `manualAnalyze` (test=5, pro=100); admin (`-1`) минует cap.
...
- **Reality-check (двойной)**:
  ...
  - **TikTok hashtags**: НЕТ Apify reality-check (дорого + slow). Вместо этого — regex post-filter `TIKTOK_HARDSKIP_RE` в `_sanitizeResponse`: ловит `kpop|kpopfyp|kpopstan|kpoptiktok|kpopedits|fandom|fandomdrama|stantwitter|stantiktok|stanstwt|celebgossip|celebtea|hollywoodgossip|gossipgirl` (anchored через `(?:^|_)…(?:_|$)`, не подстрока — `kdrama` остаётся валидным). Дроп с info-логом. Добавлено 2026-05-11 после случая `kpopfyp` + `fandomdrama` в active TikTok-тегах.
...
- **Admin Telegram digest** — **ВЫКЛЮЧЕН 2026-05-10** (`_notifyAdmins()` = no-op `return;` первой строкой; админы смотрят Pipeline page вместо дайджеста). Код функции сохранён (HTML-сводка: статус-emoji + per-preset diff, total cost, elapsed, circuit-breaker warning при streak ≥ 3, trim до 3800 char), но не вызывается.
...
**Cost**: ~$2/мес (≈$0.13/refresh × ~15 refreshes/мес — оценка из кода `tag-refresher.js`; ранний sanity-test 2026-05-07 давал ~$0.045/preset, устарело после cadence 7д→2д). Время refresh: 100-300 sec / preset × 5 ≈ 10 минут на полный прогон.
...
**Архитектура** (`src/analysis/{prompts,trigger-finder}.js`): промпт `TRIGGER_SYSTEM_PROMPT` — forecaster, не recap. Системные ограничения: ZERO references to crypto/coins/tokens/tickers/market caps. Только narrative popularity. Grok `grok-4-1-fast-reasoning` (override через `XAI_TRIGGER_MODEL` env) + `x_search` (tool). 15-min per-user cooldown УБРАН — заменён дневным cap `catalyst` из entitlements (test=5, pro=100, admin=`-1`); DB-level claim против race + `trigger_in_flight`, shared cache в `trends.trigger_*`.
...
|---|---|---|---|
| `pulse` | `#000000` (true black) | `#4ade80` (green) | **default**, `:root` baseline (без attribute) |
...
- Dashboard: `position: fixed` в sidebar nav box над Feed/Saved/Analyze. `bottom: 72px; left: 73px` (idleSitting base). Per-pose left/bottom overrides для центрирования: idleCute:103, idleHeadUp/Asleep:108, idleStayTall:109, idleLying:103+bottom 63, idleSleeping:101. Walking states left:97 (HOME_X_PX) — иначе walk-start «прыгает» от центра поз к base.
...
`src/admin/server.js` — inline React SPA. **10 табов**: 📊 Статистика / ⚙️ Сканеры / 🎛️ Пресеты / 🔄 Auto-tags / 🧪 Ручной анализ / 🔔 Алерты / 🎓 AI Examples / 👥 Пользователи / 💳 Платежи / 🤖 Бот и планы.

**Топбар (`StatusBar`)**: live-pipeline визуализация. Polls `/api/pipeline` каждые 2.5с, рендерит 8 stage-нод + 7 wires. Subtitle: `Live — Stage 1 / Stage 2...` / `Последний цикл 12с назад` / `⏸ Сканер на паузе`. На `max-width: 1100px` оборачивается под заголовком.

**Live nav indicators**: poll каждые 12с в App. Yellow pulsing dot на «Сканеры» при паузе. Accent badge на «Алерты» со счётчиком.

**Bot tab разделён на 3 sub-tabs**: 🧠 AI Pipeline / 📢 Рассылки / 💰 Планы и фидбек.
...
**Сканеры таб layout**: сверху 3 stat-cards + scanner-status-bar с Pause/Force-Scan. Ниже — 6 collapsible accordion'ов на `pcfg-accordion`: «📡 Площадки» (open) · «🎯 Конфиг сканера» (open) · «🎨 Stage 0 — PreStage» (closed) · «📤 Telegram alert pacing» (closed, per-user FIFO cooldown 60s) · «🔁 Обновление горячих трендов» (closed) · «📊 Junk-filter наблюдение» (closed). Внутренние компоненты `ScannerConfigSection / PreStageSection / AlertSchedulerSection / HotRefreshSection / JunkStatsSection` рендерят bare-div без `adm-card`.
...
| `src/config.js` | Central config — env → typed config object (порты, ключи, billing, intervals) |
...
| `src/analysis/aggregator.js` | Collect-stage aggregation/dedup entry (Aggregator → cheapDedup) |
| `src/analysis/trigger-finder.js` | Catalyst forecast engine (Grok reasoning + x_search) |
| `src/analysis/subject-names.js` | Stage 2 subject-name / nameStrength helper |
| `src/analysis/grok-cli.js` | Grok Build CLI Stage-1 provider (по подписке, 0 API-токенов) |
| `src/billing/entitlements.js` | Plan entitlements — shared by bot + dashboard |
| `src/billing/solana-pay.js` | Solana USDC payment verification (Helius) |
| `src/collectors/{reddit,twitter,tiktok,google-trends,x-trends}.js` (+ `base-collector.js` базовый класс, `twitter-check.js` reality-check) | Apify-based collectors per platform |
...
| `src/notifications/alert-scheduler.js` | Per-user FIFO alert pacing (60s cooldown, admin-tunable) |
...
| `src/refresh/tag-refresher.js` | Periodic (2д) Grok-driven tag refresh — subreddits + Twitter keywords per preset, w/ Live Search + Reddit & Apify reality-check + lock-mask |
| `src/i18n/{ru,en}.js` (+ `index.js` loader) | i18n maps для telegram + dashboard |
...
- `APIFY_API_KEY` — ещё один generic-fallback Apify token (читается в `x-trends.js`)
...
- `DASHBOARD_PORT` (code-default `3000`, в проде `8080` через `.env`) / `ADMIN_PORT` (code-default `8080`, в проде `8081` через `.env`). nginx upstream → `127.0.0.1:8080` (dashboard); admin `:8081` localhost-only. Прод-значения задаёт `.env`, НЕ дефолты кода.
