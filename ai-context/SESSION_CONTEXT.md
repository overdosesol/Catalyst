...
**TikTok engagement floor** (`tiktok.js _normalize` + `_clusterByHashtag`):
Тренд проходит если выполняется ХОТЯ БЫ ОДНО (OR-логика, идентичная Twitter):
- `plays ≥ 500 000`
- `likes ≥ 20 000`
- `shares ≥ 5 000`
- `viralScore ≥ 60` (composite — catches influencer posts)

Cluster aggregator повторяет тот же OR-floor на сумме engagement по хэштег-кластеру.
...
|---|---|---|
| Default (EN/RU/Romance/AR/...) | 500K / 10K | 500K / 20K / 5K / 60 |
| Japanese (`ja`), Korean (`ko`) | 1M / 20K (×2) | 1M / 40K / 10K / 70 |
| Chinese (`zh`) | 2M / 40K (×4) | 2M / 80K / 20K / 80 |

Все volume-метрики в TikTok скейлятся по CJK симметрично (×2 ja/ko, ×4 zh). viralScore additive (+10 для ja/ko, +20 для zh). `_detectCjkScript(text)` — kana → 'ja', hangul → 'ko', Han без них → 'zh', null иначе. Требует ≥30% CJK-доли.
